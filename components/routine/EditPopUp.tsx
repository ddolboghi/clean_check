...
import { updateContentOfMainRoutine } from "@/actions/routine";
...
export default function EditPopUp({
  ...
  const handleUpdateContent = async () => {
    handleEditBtn(editedContent);
    setOffset(0);
    if (content !== editedContent) {
      const response = await updateContentOfMainRoutine(
        routineId,
        editedContent
      );
      ...
      <div className="m-5 flex flex-col justify-between items-center p-5 bg-[#F6F9F9] min-w-[200px] w-[364px] rounded-[23px] h-[181px]">
        ...
        <div className="px-[19px] flex items-center justify-between bg-white rounded-[15px] w-full h-[46px]">
          ...
          <span className="text-[#D7D7D7] text-[12px]">
            {editedContent.length}/{ROUTINE_MAX_LENGTH}
          </span>
        </div>
        <div className="flex justify-end w-full">
          ...
          <button
            className={`${
              editedContent.length > 0 ? "bg-[#6AC7D7]" : "bg-[#C9DDE1]"
